...
def create_project_folder(fetch_mode=False):
    if fetch_mode:
        suffix = datetime.now().strftime("%Y%m%d_%H%M%S")
        rand = ''.join(random.choices(string.ascii_lowercase + string.digits, k=5))
        folder = f"project_{suffix}_{rand}"
...
if __name__ == "__main__":
    ...
    if args:
        if args[0].lower() == "fetch":
            fetch_mode = True
            args = args[1:]  # remove 'fetch' keyword
    ...
    project_folder = create_project_folder(fetch_mode=fetch_mode)
